...
from gaiacls.models.utils import DropPath
...
class ElasticConformer_stage1(BaseBackbone, DynamicMixin):
    ...
    def __init__(self,
        ...
        # Transformer
        super(ElasticConformer_stage1,self).__init__()
    ...
    def manipulate_body(self, arch_meta):
        ...
        for i, layer_name in enumerate(self.layers[:-1]):
            if i == 0:
                ...
                arch_meta_stage_tmp1['depth'] = arch_meta['depth'][i]
                arch_meta_stage_tmp1['block']['convblock']['width'] = arch_meta['block']['convblock']['width'][i]
                arch_meta_stage_tmp1['block']['embed_dim']['width'] = 384
                arch_meta_stage_tmp1['block']['transblock']['MHA']['num_heads']['num_heads'] = 6
                arch_meta_stage_tmp1['block']['transblock']['FFN']['feedforward_channels']['feedforward_channels'] = 40
                layer = getattr(self, layer_name)
                layer.manipulate_arch(arch_meta_stage_tmp1)
            if i == 1:
                arch_meta_stage_tmp2 = {'depth':1,
                        'block':{
                            'convblock':{
                                'width':96},
                            'embed_dim':{
                            ...
                            'transblock':{
                                ...
                                'FFN':{
                                    'feedforward_channels':{'feedforward_channels':40}}}}}
                arch_meta_stage_tmp2['depth'] = arch_meta['depth'][i]
                layer = getattr(self, layer_name)
                layer.manipulate_arch(arch_meta_stage_tmp2)
            if i == 2:
                arch_meta_stage_tmp3 = {'depth':1,
                        'block':{
                            'convblock':{
                                'width':192},
                            'embed_dim':{
                            ...
                            'transblock':{
                                ...
                                'FFN':{
                                    'feedforward_channels':{'feedforward_channels':40}}}}}
                arch_meta_stage_tmp3['depth'] = arch_meta['depth'][i]
                layer = getattr(self, layer_name)
                layer.manipulate_arch(arch_meta_stage_tmp3)
